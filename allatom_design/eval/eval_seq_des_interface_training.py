...
@hydra.main(config_path="../configs/eval", config_name="eval_seq_des_interface_training", version_base="1.3.2")
def main(cfg: DictConfig):
    ...
    for sd_ckpt in pbar:
        ...
        for binder_chain_id in ["A", "B"]:
            ...
            binder_chain_id_to_metrics = eval_metrics.run_af2_interface_eval(
                sampled_pdbs,
        ...
        # Aggregate results over binder chains and models
        sc_metrics = defaultdict(list)

        for record_id, metrics in id_to_metrics.items():
            for k, v in metrics.items():
                k_agg = k.split("_")[0:1]
                k_agg.extend(k.split("_")[3:])
                k_agg = "_".join(k_agg)  # e.g. binder_A_model0_i_pae -> binder_i_pae
                sc_metrics[k_agg].append(v)
