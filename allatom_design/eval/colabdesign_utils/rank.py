...
def plot_me(scores, x="tm_i", y="composite",
            title=None, diag=False, scale_axis=True, dpi=100, out_path=None,
            color_by="plddt",
            **kwargs):
  """
  Args:
  - color_by: either "plddt" or "traj"
  """
  ...
  if color_by == "plddt":
    ...
    plt.scatter(x_vals, y_vals, c=c*0.75, s=5, vmin=0, vmax=1, cmap="gist_rainbow", **kwargs)
  elif color_by == "traj":
    c = np.array([k["traj"] for k in scores])
    plt.scatter(x_vals, y_vals, c=c, s=5, cmap="gist_rainbow", **kwargs)
  else:
    assert False, f"color_by must be 'plddt' or 'traj', not {color_by}"
...
class af2rank:
  def __init__(self, pdb=None, chain=None, model_name="model_1_ptm", model_names=None, data_dir="."):
