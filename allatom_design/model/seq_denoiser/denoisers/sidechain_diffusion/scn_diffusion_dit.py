...
class SidechainDiffusionModule(nn.Module):
    ...
    def forward(self,
                ...
                z: TensorType["b n h", float],  # conditioning latent
        ...
        return x_scn, aatype, aux_preds


    def sidechain_diffusion(self,
                            z: TensorType["b n h", float],
        ...
        if not is_sampling:
            ...
            z_batched = repeat(z, "b n h -> (m b) n h", m=M, b=B)
            ...
            denoiser_fn = self.forward
            if self.use_self_conditioning and (np.random.uniform() < self.cfg.self_cond_p):
                # Apply self-conditioning
                with torch.no_grad():
                    x1_scn_batched, _, aux_preds = denoiser_fn(xt_scn_batched, aatype_batched, t_batched, z_batched, x_bb_batched,
