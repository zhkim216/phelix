...
class AtomMPNN(nn.Module):
    ...
    def forward(self, batch: dict[str, TensorType["b ..."]], is_sampling: bool):
        ...
        # Concatenate residue-level features again before passing through decoder layers
        h_V = h_V + h_S
        for layer in self.decoder_layers:
...
class TokenFeatures(nn.Module):
    def __init__(self, cfg: DictConfig):
        ...
        if self.ligand_conditioning:
            ...
            self.num_rbf * num_prot_anchor_atoms + 64 + num_angle_features, self.hidden_dim, bias=True
        )
            self.norm_nodes = torch.nn.LayerNorm(self.hidden_dim)

            # Parameters for Ligand subgraph
            # ligand subgraph nodes
            self.y_nodes = torch.nn.Linear(147, self.hidden_dim, bias=False)
...
class DecLayer(nn.Module):
    ...
    def forward(self, h_V, h_E, E_idx, mask_V=None, mask_attend=None, h_V_C_skip=None):
        ...
        if self.use_context_skip_connection:
            # Inject ligand-context refinement. Assertion in AtomMPNN.__init__ guarantees h_V_C_skip is provided.
