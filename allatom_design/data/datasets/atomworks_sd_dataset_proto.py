...
PROTO_EXTERNAL_EVIDENCE_COLUMNS = ("metal_medba_evidence", "metal_pubmed_evidence")


class AtomworksSDProtoDataModule(L.LightningDataModule):
    def __init__(self, cfg: DictConfig):
        super().__init__()
        self.cfg = cfg
        self.pdb_path = cfg.pdb_path
        self._train_set = ProtoSDDataset(cfg, phase="train")
        self._val_set = ProtoSDDataset(cfg, phase="val")
...
class ProtoSDDataset(MolecularDataset):
    def __init__(self, cfg: DictConfig, phase: Literal["train", "val"]):
        ...
        if self.phase == "train":
            ...
            dataset_name = self.proto_cfg.get("dataset_name", Path(self.metadata_path).parent.name)
            ...
            self.protein_monomer_chain_df, self.interface_df = add_proto_sampling_weights(
    ...
    def _process_train_metadata_df(self, metadata_path: str) -> pd.DataFrame:
        ...
        metadata_df = collect_external_evidence(
            metadata_df,
            require_columns=_requires_external_evidence(self.proto_cfg),
        )
        ...
        metadata_df = self._filter_metadata_to_proto_scope(metadata_df)
        return metadata_df
    ...
    def _process_protein_monomer_chain_df(self, dataset_name: str) -> pd.DataFrame:
        ...
        if self.cfg.exclude_val_cluster and self.val_cluster_ids:
            ...
            logger.info("Excluded %d proto monomer rows by val cluster.", before - len(monomer_df))

        monomer_df = add_proto_chain_counts_info(monomer_df)
    ...
    def _filter_metadata_to_proto_scope(self, metadata_df: pd.DataFrame) -> pd.DataFrame:
        protein_df = self._apply_filters(
            self.cfg.train_filters.protein_monomer_chain_filter,
            metadata_df.copy(),
        )
        protein_mask = pd.Series(metadata_df.index.isin(protein_df.index), index=metadata_df.index)
        metal_center_mask = _proto_center_mask(metadata_df, self.proto_cfg)
        keep_mask = protein_mask | metal_center_mask

        out = metadata_df[keep_mask].copy()
        if out.empty:
            raise ValueError("Proto metadata scope filter removed all rows.")
        logger.info(
            "Filtered proto train metadata to prototype scope: %d -> %d rows "
            "(protein_monomer_candidates=%d, metal_center_candidates=%d).",
            len(metadata_df),
            len(out),
            int(protein_mask.sum()),
            int(metal_center_mask.sum()),
        )
        return out
    ...
    def _process_interface_df(self, dataset_name: str) -> pd.DataFrame:
        ...
        if self.cfg.exclude_val_cluster and self.val_cluster_ids and not interface_df.empty:
            ...
            logger.info("Excluded %d proto interface rows by val cluster.", before - len(interface_df))

        interface_df = add_proto_chain_counts_info(interface_df)
    ...
    def _process_val_metadata_df(self, metadata_path: str) -> pd.DataFrame:
        ...
        if "query_pn_unit_iids" in metadata_df.columns:
            ...
            raise KeyError(
                "Proto val metadata must contain either `query_pn_unit_iids` "
                "or row-level `q_pn_unit_iid`."
            )
        metadata_df.set_index("example_id", inplace=True, drop=False, verify_integrity=True)
        logger.info("Final proto val dataset contains %d pdbs", metadata_df["pdb_id"].nunique())
        return metadata_df
    ...
    def _parse_train_dfs(self):
        ...
        logger.info(
            "Final proto train dataset has %d monomer rows and %d interface rows.",
    ...
    def _validate_sampling_weights(self):
        ...
        if len(weights) == 0:
            raise ValueError("Proto train dataset has no rows after filtering.")
        if not np.isfinite(weights).all():
            raise ValueError("Proto sampling weights contain non-finite values.")
        if (weights < 0).any():
            raise ValueError("Proto sampling weights contain negative values.")
        if weights.sum() <= 0:
            raise ValueError("Proto sampling weights have zero total mass.")

        if len(self.interface_df) > 0 and self.interface_df["sampling_weight"].sum() <= 0:
            raise ValueError("Proto interface rows exist but have zero total sampling mass.")
        if len(self.protein_monomer_chain_df) > 0 and self.protein_monomer_chain_df["sampling_weight"].sum() <= 0:
            raise ValueError("Proto monomer rows exist but have zero total sampling mass.")
    ...
    def _validate_filter_impact(self, query: str, original_num_rows: int, filtered_num_rows: int) -> None:
        if original_num_rows == 0:
            logger.warning("Query %r was applied to an empty proto dataset.", query)
            return
        if filtered_num_rows == original_num_rows:
            logger.warning("Query %r on proto dataset did not remove any rows.", query)
        elif filtered_num_rows == 0:
            raise ValueError(f"Query {query!r} on proto dataset removed all rows.")
        else:
            logger.info(
                "Query %r filtered proto dataset from %d to %d rows.",
                query,
                original_num_rows,
                filtered_num_rows,
            )
...
def collect_external_evidence(
    ...
    evidence_columns: tuple[str, ...] = PROTO_EXTERNAL_EVIDENCE_COLUMNS,
    output_col: str = "has_external_evidence",
    require_columns: bool = True,
) -> pd.DataFrame:
    """Attach a boolean external-evidence flag for metal PN-units."""
    out = metadata_df.copy()
    missing = [col for col in evidence_columns if col not in out.columns]
    if missing and require_columns:
        raise KeyError(f"Proto external evidence requires missing columns: {missing}")

    evidence = pd.Series(False, index=out.index)
    for col in evidence_columns:
        if col in out.columns:
            evidence = evidence | out[col].fillna(False).astype(bool)

    is_metal = out.get("q_pn_unit_is_metal", pd.Series(False, index=out.index)).fillna(False).astype(bool)
    out[output_col] = is_metal & evidence
    return out


def build_proto_interface_df(
    metadata_df: pd.DataFrame,
        ...
        row.update(
            {
                ...
                "ligand_ccd_key": _normalize_ligand_ccd_key(center.q_pn_unit_non_polymer_res_names),
...
def add_proto_chain_counts_info(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
...
def _requires_external_evidence(proto_cfg: dict | DictConfig) -> bool:
    proto_cfg = proto_cfg or {}
    policy = proto_cfg.get("external_evidence_policy", "no_filter")
    if policy == "no_filter":
        return False
    if policy == "external_evidence":
        return True
    raise ValueError(
        f"Unknown proto external_evidence_policy={policy!r}. "
        "Supported values: 'no_filter', 'external_evidence'."
    )


def _get_proto_allowed_codes(proto_cfg: dict | DictConfig) -> set[str]:
    proto_cfg = proto_cfg or {}
    raw_codes = proto_cfg.get("allowed_ccd_codes", None)
    code_set = _normalize_ccd_codes(raw_codes)
    if raw_codes is not None and not code_set:
        raise ValueError("Proto `allowed_ccd_codes` must be null or contain at least one exact CCD code.")
    return code_set


def _proto_center_mask(metadata_df: pd.DataFrame, proto_cfg: dict | DictConfig) -> pd.Series:
    required_cols = [
        "q_pn_unit_is_metal",
        "q_pn_unit_non_polymer_res_names",
        "q_pn_unit_avg_occupancy_nonpolymer",
    ]
    requires_external_evidence = _requires_external_evidence(proto_cfg)
    if requires_external_evidence:
        required_cols.append("has_external_evidence")
    missing = [col for col in required_cols if col not in metadata_df.columns]
    if missing:
        raise KeyError(f"Proto center mask missing required columns: {missing}")

    proto_cfg = proto_cfg or {}
    allowed_codes = _get_proto_allowed_codes(proto_cfg)
    min_occupancy = float(proto_cfg.get("min_avg_occupancy_nonpolymer", 0.5))
    ...
    if requires_external_evidence:
        mask = mask & metadata_df["has_external_evidence"].fillna(False).astype(bool)
    return mask
