...
#
# === Original (Atomworks) AF3-like spatial crop + atom-budget resize ===
# Copied from `atomworks/ml/transforms/crop.py` to keep the baseline behavior available
# alongside the constrained variant(s).
#
...
def crop_spatial_like_af3(
    ...
    """
    Original AF3-like spatial crop: pick a crop center from query pn_units and take nearest neighbors.
    """
...
def resize_crop_info_if_too_many_atoms(
    ...
    """
    Original AF3-like atom-budget resize: drop farthest tokens until within `max_atoms`.
    """
    ...
    cutoff = np.max(np.where(within_budget)) + 1
...
class CropSpatialLikeAF3(CropTransformBase):
    """
    Original Atomworks CropSpatialLikeAF3 (kept for baseline behavior).
    """
...
def crop_spatial_like_af3_constrained(
    atom_array: AtomArray,
    query_pn_unit_iids: list[str],
    crop_size: int,
    jitter_scale: float = 1e-3,
    ...
    if force_crop or requires_crop:
        ...
        # Compute token-level distances to crop center (with optional jitter to break ties)
        coord_for_dist = token_coords
        if jitter_scale > 0:
            coord_for_dist = coord_for_dist + np.random.normal(scale=jitter_scale, size=coord_for_dist.shape)
        crop_center_coord = coord_for_dist[crop_center_token_idx]
        is_valid = np.isfinite(coord_for_dist).all(axis=1)
        # If crop center somehow has invalid coords, fall back to vanilla mask selection
        if not np.isfinite(crop_center_coord).all():
            is_token_in_crop = get_spatial_crop_mask(
                token_coords, crop_center_token_idx, crop_size=crop_size, jitter_scale=jitter_scale
            )
            is_atom_in_crop = spread_token_wise(atom_array, is_token_in_crop, token_starts=token_segments)
            protected_token_idxs = np.array([], dtype=int)
        else:
            ...
            protected = list(dict.fromkeys(protected))  # stable-unique

            # If protected already exceeds crop_size, truncate (keep closest-to-center protected first)
            if len(protected) > crop_size:
                protected_sorted = sorted(protected, key=lambda t: dists[t] if np.isfinite(dists[t]) else np.inf)
                protected = protected_sorted[:crop_size]

            # Fill remaining budget by nearest tokens overall
            is_token_in_crop = np.zeros((n_tokens,), dtype=bool)
            is_token_in_crop[protected] = True
            remaining = crop_size - int(is_token_in_crop.sum())
            if remaining > 0:
                order = np.argsort(dists)  # invalids are inf -> go to end
                for t in order:
                    if remaining == 0:
                        break
                    if not np.isfinite(dists[t]):
                        break
                    if is_token_in_crop[t]:
                        continue
                    is_token_in_crop[t] = True
                    remaining -= 1

            # Only protect the explicitly-reserved tokens (NOT the entire crop).
            # This allows atom-budget resizing to drop non-protected tokens while keeping constraints.
            protected_token_idxs = np.array(protected, dtype=int)
    ...
    return {
        ...
        # Token-level indices we try to guarantee (used to protect constraints during atom-budget resizing)
        "protected_token_idxs": protected_token_idxs,
    }


def resize_crop_info_like_af3_constrained(
    crop_info: dict,
    atom_array: AtomArray,
    max_atoms: int,
) -> dict:
    ...
    # Subset to tokens within budget, optionally protecting some token indices.
    protected_val = crop_info.get("protected_token_idxs", None)
    if protected_val is None:
        protected_token_idxs: set[int] = set()
    else:
        # `protected_token_idxs` can be a numpy array; avoid `or []` ambiguity.
        protected_token_idxs = set(map(int, np.asarray(protected_val).tolist()))
    protected_in_crop = np.array([t in protected_token_idxs for t in crop_token_idxs], dtype=bool)

    # If protected tokens alone exceed budget, we fall back to original behavior.
    protected_atoms = int(np.sum(n_atoms_per_token[protected_in_crop])) if protected_in_crop.any() else 0

    if protected_atoms > max_atoms:
        logger.warning(
            f"Protected tokens require {protected_atoms} atoms which exceeds max_atoms={max_atoms}; "
            "falling back to distance-only resizing."
        )
        cutoff = np.max(np.where(within_budget)) + 1
        is_in_budget = sort_by_distance[:cutoff]
    else:
        chosen = []
        # Always keep protected tokens first
        if protected_in_crop.any():
            chosen.extend(list(np.where(protected_in_crop)[0]))

        # Then fill by increasing distance among remaining tokens
        remaining_budget = max_atoms - protected_atoms
        # Candidate order by distance (already on crop_atom_array token index space)
        for idx in sort_by_distance:
            if idx in chosen:
                continue
            n_atoms = int(n_atoms_per_token[idx])
            if n_atoms <= 0:
                continue
            if n_atoms > remaining_budget:
                continue
            chosen.append(int(idx))
            remaining_budget -= n_atoms
            if remaining_budget <= 0:
                break

        is_in_budget = np.array(chosen, dtype=int)
...
class CropSpatialLikeAF3Constrained(CropTransformBase):
    """
    ...
    def forward(self, data: dict) -> dict:
        ...
        #! (JH) changed
        # Prefer explicitly-provided crop-center candidates for interface examples.
        # We keep `query_pn_unit_iids` for "allowed PN-units after processing" (used by FilterToQueryPNUnits),
        # but want the crop center to be sampled only from the interface pair when available.
        if data.get("crop_center_pn_unit_iids"):
            query_pn_units = data["crop_center_pn_unit_iids"]
        elif data.get("q_pn_unit_iid_1") and data.get("q_pn_unit_iid_2"):
            query_pn_units = [data["q_pn_unit_iid_1"], data["q_pn_unit_iid_2"]]
        ...
        crop_info = crop_spatial_like_af3_constrained(
            atom_array=atom_array,
            query_pn_unit_iids=query_pn_units,
            crop_size=self.crop_size,
            jitter_scale=self.jitter_scale,
