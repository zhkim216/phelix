...
@hydra.main(config_path="../../../configs/data/preprocessing/boltz", config_name="pretokenize_inputs", version_base="1.3.2")
def main(cfg: DictConfig):
    ...
    processed_structure_files = glob.glob(f"{cfg.pdb_path}/rcsb_processed_targets/structures/*.npz")

    # Tokenize each structure
    use_parallel = cfg.num_workers > 1
    if use_parallel:
        with Parallel(n_jobs=cfg.num_workers) as parallel_pool:
            jobs = [delayed(tokenize_structure_to_disk)(processed_structure_file, out_dir, tokenizer) for processed_structure_file in processed_structure_files]
        ...
        for processed_structure_file in tqdm(processed_structure_files, desc="Tokenizing structures"):
            tokenize_structure_to_disk(processed_structure_file, out_dir, tokenizer)


def tokenize_structure_to_disk(processed_structure_file: str, out_dir: str, tokenizer: Tokenizer) -> None:
    """
