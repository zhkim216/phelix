...
def main(cfg: DictConfig):
    ...
    and writes a pdb manifest CSV containing useful information, including pdb_name, phase, seq_length, and cluster_id.
    """
    ...
    #     pdb_key_to_length = get_lengths_from_cached(pdb_keys, cache_dir)
...
def cache_examples(
    ...
    jobs = [
        delayed(process_pdb_key)(pdb_key, cache_dir, overwrite_cache, pdb_path, phase)
        for pdb_key in pdb_keys
    ]
    for _ in tqdm(parallel(jobs), total=len(jobs), desc="Caching PDBs"):
        pass
...
def get_lengths_from_cached(pdb_keys: List[str], cache_dir: str) -> Dict[str, int]:
    """
...
def process_pdb_key(pdb_key: str, cache_dir: str, overwrite_cache: bool, pdb_path: str, phase: str):
