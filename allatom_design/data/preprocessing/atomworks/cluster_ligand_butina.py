...
Why complete-linkage (and not Butina anymore)?
    Butina is a sparse-greedy single-pass clusterer: a point joins the first
    centroid that is within ``cutoff`` of it. As a consequence, the diameter
    (largest intra-cluster pairwise distance) of a Butina cluster can grow well
    beyond the nominal cutoff — on our ATP analysis we observed cluster
    diameters of up to 1.67x the cutoff, meaning two members of the "same"
    cluster shared Tanimoto similarity well below the requested 0.8 threshold.
...
Module name is preserved as ``cluster_ligand_butina`` for v8_pipeline.sh
backward compatibility — only the algorithm has changed.
...
def load_smiles_cache(cache_path: Path) -> dict[str, str | None]:
    """Load the Step-0 SMILES cache JSON (CCD code -> SMILES string or None)."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "ECFP4 fingerprints with Tanimoto distance. (Module name retained "
            "as cluster_ligand_butina for v8_pipeline.sh compatibility.)"
        )
    )
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help="Step-0 SMILES cache, e.g. ccd_smiles_cache_metadata_v8.json",
    )
