...
@dataclass(frozen=True)
class DesignabilityInfo:
    """
    Contains various designability metrics for the target.
    """
    # self-consistency
    sc_ca_rmsd: float
    sc_ca_tm: float
...
class Record(JSONSerializable):
    ...
    designability_info: Optional[DesignabilityInfo] = None
...
class Manifest(JSONSerializable):
    ...
    @classmethod
    def load(cls: "JSONSerializable", path: str | Path) -> "JSONSerializable":
        """Load the object from a JSON file.
