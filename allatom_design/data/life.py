# Source: https://github.com/lucidrains/alphafold3-pytorch/blob/308c470a0e2d8c9ea80cdd3e25149af0a7de5a60/alphafold3_pytorch/life.py#L5
...
def reverse_complement_tensor(t):  # type: ignore
...
# Add CA-centered reference positions to entries
for entry in HUMAN_AMINO_ACIDS.values():
    mol = entry["rdchem_mol"]
    CA_idx = rc.restype_name_to_atom14_names[entry["resname"]].index("CA")
    ...
    # Center on CA
    all_atom_pos_tensor = all_atom_pos_tensor - all_atom_pos_tensor[CA_idx:CA_idx + 1]

    # Pad to atom14 format
    ref_pos = torch.zeros(14, 3)
    ref_pos[:all_atom_pos_tensor.shape[0]] = all_atom_pos_tensor

    # Add to entry
    entry["ref_pos_atom14"] = ref_pos


def _make_restype_ref_pos_atom14() -> TensorType["21 14 3"]:
    """
    Make reference positions for each amino acid residue in atom14 format.
    """
    restype_ref_pos_atom14 = torch.zeros(21, 14, 3)
    for i, rt in enumerate(rc.restypes_with_x):
        entry = HUMAN_AMINO_ACIDS[rt]
        restype_ref_pos_atom14[i] = entry["ref_pos_atom14"]
    return restype_ref_pos_atom14


def _make_restype_ref_pos_atom37(restype_ref_pos_atom14) -> TensorType["21 37 3"]:
    """
    Make reference positions for each amino acid residue in atom37 format.
    """
    restype_ref_pos_atom37 = torch.gather(
        restype_ref_pos_atom14,
        1,
        torch.tensor(rc.RESTYPE_ATOM37_TO_ATOM14[..., None], dtype=torch.long).expand(-1, -1, 3),
    )
    restype_ref_pos_atom37 = restype_ref_pos_atom37 * torch.tensor(rc.restype_atom37_mask)[..., None]
    return restype_ref_pos_atom37


RESTYPE_REF_POS_ATOM14 = _make_restype_ref_pos_atom14()
RESTYPE_REF_POS_ATOM37 = _make_restype_ref_pos_atom37(RESTYPE_REF_POS_ATOM14)
