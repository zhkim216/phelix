...
# Build the Elix Apptainer image on Sherlock.
# Run from any directory after cloning/pulling the elix repo.
...
SIF="${SIF:-$ELIX_SCRATCH/containers/elix.sif}"
DEF="${DEF:-$SCRIPT_DIR/elix_apptainer.def}"
...
BUILD_DEF="$(mktemp "${TMPDIR:-/tmp}/elix_apptainer.XXXXXX.def")"
trap 'rm -f "$BUILD_DEF"' EXIT
awk -v patch_b64="$PATCH_B64" '
  { gsub(/__ELIX_JACKHMMER_PATCH_B64__/, patch_b64); print }
' "$DEF" > "$BUILD_DEF"

echo "Building Elix SIF"
echo "  repo: $REPO_ROOT"
echo "  def:  $DEF"
