...
#
# Usage:
#   bash scripts/sherlock_scripts/jinho/eval_seq_des_training/merge_pocket_constraint_shards.sh <output_dir>
#
# Example:
#   bash merge_pocket_constraint_shards.sh /scratch/users/zhkim216/out_dir/eval_ligand_seq_des/pocket_pos_constraint_mad
# ==============================================================================
...
for K in "${K_VALUES[@]}"; do
    # Merge minimal CSVs
    MINIMAL_SHARDS=(${OUTPUT_DIR}/pocket_constraint_mad_k${K}_shard*[0-9].csv)
    if [ ${#MINIMAL_SHARDS[@]} -eq 0 ] || [ ! -f "${MINIMAL_SHARDS[0]}" ]; then
        echo "Warning: no shard files found for k=${K} (minimal)"
        continue
    fi

    MERGED_MINIMAL=${OUTPUT_DIR}/pocket_constraint_mad_k${K}.csv
    ...
    # Merge full CSVs
    FULL_SHARDS=(${OUTPUT_DIR}/pocket_constraint_mad_k${K}_shard*_full.csv)
    if [ ${#FULL_SHARDS[@]} -eq 0 ] || [ ! -f "${FULL_SHARDS[0]}" ]; then
        echo "Warning: no shard files found for k=${K} (full)"
        continue
    fi

    MERGED_FULL=${OUTPUT_DIR}/pocket_constraint_mad_k${K}_full.csv
