...
REPO_DIR="/home/users/zhkim216/code/allatom-design"
ENV_DIR="$SCRATCH/venv"
# ----------------------------------------------------
...
"$APPTAINER_BIN" exec $NV_FLAG \
  --env TRITON_LIBCUDA_PATH=/usr/local/cuda/compat/lib.real/lib \
  --bind "$BIND_LIST" "$IMG" \
  bash -lc '
    set -euo pipefail
